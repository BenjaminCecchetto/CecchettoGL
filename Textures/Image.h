...
class ImageReadError : public std::exception {
 ...
 public:
  ImageReadError(uint32_t i, uint32_t j, uint32_t c) : _i(i), _j(j), _c(c) {}
...
class Image : public TypedBufferData<T> {
 ...
 public:
  ...
  Image(uint32_t width, uint32_t height, uint32_t channels)
      : TypedBufferData<T>(width * height * channels),
        _width(width),
        _height(height),
        _channels(channels) {}
  ...
  T& At(uint32_t i, uint32_t j, uint32_t c) {
    if (i < _height && j < _width && c < _channels) {
      return TypedBufferData<T>::_data[(j + i * _width) * _channels + c];
    } else {
